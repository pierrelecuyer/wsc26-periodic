...
The present repository contains an Online Supplement for the paper that provides the proof of Proposition 2 and the variance plots, links to the Java code and tools used to make the experiments, and the output data files.  The Supplement is in file wsc26periodic-Supplement.pdf. The data files are in the directory `results64-june`. 

The main Java classes used to make the experiments are in files WSC26RepsRQMC64.java and WSC26RQMCSamples64.java. These two files are in the `https://github.com/pierrelecuyer/ssj/tree/develop/src/main/docs/examples/rqmcexperiments` directory of the [SSJ library](https://github.com/pierrelecuyer/ssj). The model implementations are also in that same directory. To run this code, one must first install SSJ by following the instructions given in the README of the GitHub site of SSJ. 
